...
class KnowledgeArticle(BaseEntity, db.Model):
    ...
    knowledge_article_id = db.mapped_column(db.Integer, primary_key=True,
                                     autoincrement=True)
    knowledge_article_title = db.mapped_column(db.String(100), nullable=False)
    knowledge_article_content = db.mapped_column(db.Text, nullable=False)
    knowledge_article_category_id = db.mapped_column(
        db.Integer, db.ForeignKey('categories.category_id'),
        nullable=False
    )
    knowledge_article_author_id = db.mapped_column(db.Integer, db.ForeignKey('users.user_id'),
                          nullable=False)
