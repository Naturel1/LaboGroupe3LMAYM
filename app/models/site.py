...
class Site(BaseEntity, db.Model):
    ...
    site_id = db.mapped_column(db.Integer, primary_key=True, autoincrement=True)
    site_name = db.mapped_column(db.String(100), nullable=False)
    site_address = db.mapped_column(db.String(255), nullable=True)
    site_city = db.mapped_column(db.String(100), nullable=True)
